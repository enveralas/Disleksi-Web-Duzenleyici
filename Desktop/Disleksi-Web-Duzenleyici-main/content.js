...
if (!document.getElementById("dyslexia-style")) {
  ...
  style.innerHTML = `
    ...
    .dyslexia-replaced-text {
      ...
      font-family:
        var(--d-font, OpenDyslexic) !important;
...
chrome.storage.sync.get(
  ...
  (res) => {
    ...
    if (res.fontFamily) {
      currentFont = res.fontFamily;
    }

    if (res.fontSize) {
      currentSize = parseInt(res.fontSize);
    }
  }
);
...
function replaceSelectedText(
  ...
  if (!lastRange) {
    ...
    alert(
      "Metin seçimi bulunamadı. Lütfen metni tekrar seç."
    );
